...
inline bool bitnmask_check_straight(int* hand);
...
bool bitnmask_check_straight(int* hand) {
	// Initialize bitmasks to represent the hand with both high and low aces
	int bitmask_high_ace = 0;
	int bitmask_low_ace = 0;

	// Set the corresponding bit for each card value in the hand
	for (int i = 0; i < 5; ++i) {
		bitmask_high_ace |= (1 << hand[i]);
		bitmask_low_ace |= (1 << (hand[i] == 14 ? 1 : hand[i]));  // Treat ace as low
	}

	// Check if the hand contains a straight with high ace
	bool straight_with_high_ace = (bitmask_high_ace & (bitmask_high_ace << 1) &
		(bitmask_high_ace << 2) & (bitmask_high_ace << 3) &
		(bitmask_high_ace << 4)) != 0;

	// Check if the hand contains a straight with low ace
	bool straight_with_low_ace = (bitmask_low_ace & (bitmask_low_ace << 1) &
		(bitmask_low_ace << 2) & (bitmask_low_ace << 3) &
		(bitmask_low_ace << 4)) != 0;

	// Return true if either straight with high ace or straight with low ace is found
	return straight_with_high_ace || straight_with_low_ace;
}
